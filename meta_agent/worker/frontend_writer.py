	...
	def _build_user_prompt(
		...
		*,
		...
		return (
			...
			"22) Before submitting any step, always serialize complex UI control state into a JSON string for the `input` field: multi-select chips → JSON.stringify({field: selectedArray}); line-list / bullet-point textarea → JSON.stringify({field: lines.filter(Boolean)}); tag/pill lists → JSON.stringify({field: tags}). Never send raw JS arrays or objects. Single plain-text inputs submit their string value directly.\n\n"
